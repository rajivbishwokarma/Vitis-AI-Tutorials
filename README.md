<table style="width:100%">
  <tr>
    <th width="100%" colspan="6"><img src="https://www.xilinx.com/content/dam/xilinx/imgs/press/media-kits/corporate/xilinx-logo.png" width="30%"/><h1>MNIST Classification using Vitis AI and TensorFlow</h2>
</th>
  </tr>
...
The MNIST handwritten digits dataset is a publicly available dataset that contains a total of 70k 8bit grayscale images each of which are 28pixels x 28pixels. The dataset is considered to be the 'hello world' of machine learning. The complete dataset of 70k images is normally divided into 60k images for training and 10k images for validation.
...
## 5. Shell scripts
  ...
  + ``0_setenv.sh`` : Sets all necessary environment variables used by the other scripts. Most variables can be edited by the user to configure the environment for their own requirements, for example the targetted evaluation board type can be changed by editing the ``export BOARD=ZCU102`` variable.
 ...
 It is highly recommended to leave the ``CALIB_IMAGES`` variable set to 1000 as this is the minimum recommended number of images for calibration of the quantization.
  ...
  + ``7_make target.sh`` : Copies the .elf and images to the target folder ready to be copied to the evaluation board's SD card. The .elf file will be converted to a Shared Library/Static Object .so file using the ARM compiler on the evaluation board.
...
If you are running the training step, it is highly recommended to use the GPU version of the Vitis Tools Docker container.
...
..where -t defines the number of threads and -b defines the input batch size. The -j option gives the path of the folder that contains the meta.json file.
