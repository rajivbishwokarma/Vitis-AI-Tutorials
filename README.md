# DenseNetX - a version of DenseNet modified for use with Xilinx DPUv2 on Zynq


### Current status

- [X] Tested on ZCU102 with Vitis-AI 1.1 and VART runtime
...
We will run the following steps:
  ...
  + Download and execution of the application on an evaluation board.
...
The host machine has several requirements that need to be met before we begin. You will need:
  ...
  + A target board - in this tutorial, the Xilinx ZCU102 board is used. Users should follow all of the steps described in [Setting Up the Evaluation Board](https://www.xilinx.com/html_docs/vitis_ai/1_1/yjf1570690235238.html) section of the User Guide, including the installation of the VART runtime.
...
The generated instructions are specific to the particular configuration of the DPU. The DPU's parameters are contained in a .dcf file which needs to be created for each target board - see thw Vitis-AI User Guide for details.
...
This message tells us that we have an unsupported layer, Softmax in this case, in the quantized model. The Vitis-AI compiler will ignore this layer and only compile the network up to the output of the previous layer. Generally, for unsupported layers the user must provide an equivalent software function that will be executed on the CPU - but be aware that in the particular case of Softmax, the DPUv2 has an optional hardware block to implement it.
